...
func TestCreateProduct(t *testing.T) {
	...
	if m["name"] != "test product" {
		t.Errorf("Expected the 'name' key of the response to be set to 'test product'. Got %v ", m["name"])
	}

	if m["price"] != 11.22 {
		t.Errorf("Expected the 'price' key of the response to be set to '11.22'. Got %v ", m["price"])
	}
...
func TestUpdateProduct(t *testing.T) {
	...
	if m["price"] == originalProduct["price"] {
		t.Errorf("Expected the 'price' to change from %v to %v ", originalProduct["price"], m["price"])
	}
}
